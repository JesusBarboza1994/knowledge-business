 ...
 *
 * When mcp-remote or Streamable HTTP gain full prompt/resource support,
 * these tools can be removed — the originals in kb.prompt.ts / kb.resource.ts
 * will take over.
 */
@Injectable()
    ...
    return [
      {
        ...
        description:
          'Call BEFORE ingesting a document. Returns the workflow and rules for decomposing documents into structured wiki notes. No arguments needed.',
        schema: {},
        handler: async () => ({
          guide: INGEST_GUIDE,
        }),
      },
      {
        name: 'kb_query_guide',
        description:
          'Call BEFORE answering a user question from the wiki. Returns the search strategy and answer rules tailored to the question.',
      ...
      },
      {
        name: 'kb_lint_guide',
        description:
          'Call to run a health check on the wiki. Returns the checklist for finding orphans, broken links, missing provenance, and contradictions.',
        schema: {},
        handler: async () => ({
          guide: LINT_GUIDE,
        }),
      },
      {
        name: 'kb_whoami',
        description:
          'Returns current user identity, accessible areas, permissions, and note counts per area. Call this to understand your scope before working.',
        schema: {},
...
// ─── Guide texts (same content as kb.prompt.ts) ────────────────────────────

const INGEST_GUIDE = `You are ingesting a document into the Knowledge Hub. The wiki is a **compiled knowledge base** — ingesting is not just indexing, it means reading, understanding, and **integrating** the source into the existing body of knowledge.
...
const QUERY_GUIDE = `Answer this question using the Knowledge Hub: "\${question}"
...
const LINT_GUIDE = `Run a structural health check on the Knowledge Hub wiki. Report issues and optionally fix them.
